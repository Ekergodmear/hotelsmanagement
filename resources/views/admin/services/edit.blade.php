            ...
            <form action="{{ route('admin.services.update', $service) }}" method="POST">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control @error('type') is-invalid @enderror"
                        ...
                        <option value="room_service" {{ old('type', $service->type) == 'room_service' ? 'selected' : '' }}>
                            Dịch vụ phòng
                        </option>
                        <option value="food_beverage" {{ old('type', $service->type) == 'food_beverage' ? 'selected' : '' }}>
                            Đồ ăn & Thức uống
                        </option>
                        <option value="spa" {{ old('type', $service->type) == 'spa' ? 'selected' : '' }}>
                            Spa & Massage
                        </option>
                        <option value="laundry" {{ old('type', $service->type) == 'laundry' ? 'selected' : '' }}>
                            Giặt ủi
                        </option>
                        <option value="other" {{ old('type', $service->type) == 'other' ? 'selected' : '' }}>
                            Khác
                        </option>
                    </select>
                    @error('type')
